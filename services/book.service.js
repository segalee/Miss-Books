...
function getRequestedBookId(bookId, diff) {
    const books = _loadBooksFromStorage();
    const bookIdx = books.findIndex(book => book.id === bookId)
    let requestedBookIdx = bookIdx + diff;
    if (requestedBookIdx === books.length) requestedBookIdx = 0
    else if (requestedBookIdx === -1) requestedBookIdx = books.length - 1
    console.log('requestedBookIdx:', requestedBookIdx);
    console.log('books[requestedBookIdx].id:', books[requestedBookIdx].id);

    return books[requestedBookIdx].id
}

function getSearchedBooks(query) {
